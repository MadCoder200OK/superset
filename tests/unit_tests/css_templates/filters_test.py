...
def test_apply_returns_query_unchanged_for_empty_value() -> None:
    mock_query = MagicMock()
    filt = _make_filter()
    result = filt.apply(mock_query, "")
    assert result is mock_query
    mock_query.filter.assert_not_called()


def test_apply_returns_query_unchanged_for_none_value() -> None:
    mock_query = MagicMock()
    filt = _make_filter()
    result = filt.apply(mock_query, None)
    assert result is mock_query
    mock_query.filter.assert_not_called()


def test_apply_calls_filter_for_nonempty_value() -> None:
    mock_query = MagicMock()
    filt = _make_filter()
    filt.apply(mock_query, "bootstrap")
