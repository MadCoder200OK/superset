...
def test_valid_sql_returns_no_annotations() -> None:
...
def test_invalid_sql_returns_annotation_with_line_number() -> None:
    mock_database = MagicMock()
    with patch(
        "superset.sql_validators.postgres.check_string",
    ...
    assert annotations[0].message == 'syntax error at or near "SELEC"'
